...
public class UserCtrl {
	...
	@RequestMapping(value="/login", method=RequestMethod.GET)
	 public String pushLogin(UserVO user, StoreVO store, Model model, RedirectAttributes rttr) {
	  System.out.println("UserCtrl pushLogin()");
	  System.out.println(user.getEmployee_name());
	  ...
	  if(result!=null) {
	   ...
	   return "redirect:/dashboard/main?id="+store.getStore_id(); // return을 interceptor에게 넘겼다!
	  } else {
	   // 만약 로그인페이지로 돌아가면서 하면서 id, pwd 값을 유지한다면? 
	   System.out.println("no user data");
	   return "redirect:/"; // return을 interceptor에게 넘겼따.
